...
namespace Weather.Consoles
{
    ...
    class Program
    {
        ...
        public async Task PrintForecastToConsole(Forecast f)
        {
                var grouping = f.Items.GroupBy(x => x.dateTime.Date);
                foreach (var i in grouping)
                {
                    theConsoleString.Append("\n" + i.Key.ToShortDateString());
                    foreach (var j in i)
                    {
                        theConsoleString.Append("\n\t--" + j.dateTime.ToShortTimeString() +
                            $", Temp: {j.Temperature}, " +
                            $"Wind Speed: {j.WindSpeed}, " +
                            $"Description: {j.Description}");
                    }
                }
            await Task.Delay(500);
            theConsole.Write(theConsoleString.ToString());
        }
